"""Launch Ohio g5.2xlarge to run held-out HF eval for FinnAI SLM v2."""
...
# v2 dataset (includes Indic test items)
aws s3 sync "s3://$BUCKET/datasets/finai-slm/v2" "$WORKDIR/data" --region ap-south-1
aws s3 cp "s3://$BUCKET/code/finai-slm/evaldir-v2.tar.gz" /tmp/eval.tgz --region ap-south-1
...
# v2 all 3 epochs tied at 0.965; use merged from training (epoch 3)
aws s3 sync "s3://$BUCKET/jobs/finai-slm/$TRAIN_JOB/merged" "$WORKDIR/merged-v2" --region ap-south-1
...
def main() -> None:
    ...
    job = f"finnai-eval-v2-ohio-{stamp}"
    s3 = boto3.client("s3", region_name=DATA_REGION)
    ec2 = boto3.client("ec2", region_name=REGION)
    s3.put_object(
        Bucket=BUCKET,
        Key="code/finai-slm/evaldir-v2.tar.gz",
        Body=_tar(),
        ServerSideEncryption="AES256",
    )
    print("uploaded evaldir-v2")
